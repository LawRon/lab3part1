...
unsigned int incorrect_branch_predictions;
...
void iplc_sim_LRU_replace_on_miss( int index, int tag )
{
    ...
    j=cache[index].replacement[cache_assoc-1];
    cache[index].replacement[cache_assoc-1]=i;
    int temp=0;
    for(int d=cache_assoc-2;d>=0;d--){
    	
    	temp=cache[index].replacement[d];
    	cache[index].replacement[d]=j;
    	j=temp;
...
void iplc_sim_LRU_update_on_hit( int index, int assoc )
{
	...
	int temp = cache[index].replacement[cache_assoc-1];
	int temp2 = 0;
	cache[index].replacement[cache_assoc-1] = cache[index].replacement[j];
	for ( i = cache_assoc-2; i >= j; i--) 
	{
		temp2 = cache[index].replacement[i];
		cache[index].replacement[i] = temp;
		temp = temp2;
	}
}
...
int iplc_sim_trap_address( unsigned int address )
{
    ...
    int hit=0;
	int assoc=0;
    unsigned int index_mask = ~(~(0) << cache_index) << cache_blockoffsetbits;
	unsigned int tag_mask = ~(0) << (cache_index + cache_blockoffsetbits);
	
	index = (address & index_mask) >> cache_blockoffsetbits;//Mask the index and tag out of the address
	tag = (address & tag_mask) >> (cache_index + cache_blockoffsetbits);
	
	printf("Address 0x%x: Tag = 0x%x Index = 0x%x \n",address,tag,index);
	
	cache_access++;
	
	for(i = 0; i < cache_assoc; i++)//Check each assoc's tag and valid bit
	{
    ...
    if(hit == 1)
	{
		iplc_sim_LRU_update_on_hit( index, assoc);
		cache_hit++;
	}
	else
	{
...
void iplc_sim_push_pipeline_stage()
{
	...
	{
		if( pipeline[ALU].itype == RTYPE && pipeline[MEM].stage.lw.dest_reg==pipeline[ALU].stage.rtype.reg1) pipeline_cycles += 1;
        else if( pipeline[ALU].itype == RTYPE && pipeline[MEM].stage.lw.dest_reg==pipeline[ALU].stage.rtype.reg2_or_constant) pipeline_cycles += 1;
	}
	...
	{
		branch_count++;
        if (pipeline[FETCH].itype==NOP){incorrect_branch_predictions++;}

		else if( branch_predict_taken == 1 && (pipeline[FETCH].instruction_address-4)== pipeline[DECODE].instruction_address )
		{
			pipeline_cycles += 1;
			incorrect_branch_predictions++;
		}
		else if( branch_predict_taken == 0 && (pipeline[FETCH].instruction_address-4)!= pipeline[DECODE].instruction_address )
		{
			pipeline_cycles += 1;
			incorrect_branch_predictions++;
		}
